...
# purpose: builds formula to be used in gam()
# input: response variables Y, predictors vector x
...
#' @export
#' 
prune_dag <- function(data, curr_amat, alpha=0.00001, standardized=FALSE, cluster=NULL, nodelabels=colnames(data)){
  ...
  return(list(amat=amat(curr_graph), cond_set=cond_set, total_time=total_time))
}
